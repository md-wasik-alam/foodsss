...
const Menu = ({name,img,price,data},props) => {
  ...
  return (
    <div className='text-white rounded-lg  bg-gray-200 hover:bg-gray-300'>
      <img src={img} width="230px" className="object-cover h-40 rounded-t-lg" alt={name} />
      <div className="flex flex-col text-center">
        <h4 className=' font-semibold text-xl text-gray-800 hover:text-gray-900 text-left px-3 py-1' >{name}</h4>
        <h5 className='font-semibold text-lg text-red-600 hover:text-xl hover:text-red-500'>Price/- <span className='text-green-700 hover:text-green-600'>{price}</span></h5>
      </div>
       ...
       <Dialog open={open} onClose={handleClose}>  
        ...
        <DialogContent>
          <div className=" ">
            ...
            <img src={img} width="100%"  className="object-cover h-52 mt-3 rounded-lg" alt={name} />
            </div>
          <div className="flex flex-col text-center mt-12">
            <h4 className=' font-semibold text-xl text-gray-800 hover:text-gray-900 text-center px-3 py-1' >{name}</h4>
            <h5 className='font-semibold text-lg text-red-600 hover:text-xl hover:text-red-500'>Price/- <span className='text-green-700 hover:text-green-600'>{price}</span></h5>
         ...
         {/* dialog table  */}
          <table className="table table-dark table-hover table-bordered table-striped ">
            ...
            <tbody>
                      <tr>
                        <td className='hover:first-letter:text-red-600 first-letter:text-xl first-letter:text-green-400 hover:font-semibold w-48 hover:text-center first-letter:capitalize  cursor-pointer'>{name}</td>
                        <td className='hover:first-letter:text-red-600 first-letter:text-green-400 text-xl text-center cursor-pointer'>{count}</td>
                        <td className='hover:first-letter:text-red-600 first-letter:text-xl first-letter:text-green-400 text-center cursor-pointer'>Rs. {count*price}</td>
                        <td className='text-center cursor-pointer '><span className='hover:text-red-600 hover:bg-black active:bg-green-400 cursor-pointer px-2 py-1 rounded-md text-center bg-red-400 font-bold'>X</span></td>
                      </tr>
         ...
         {/*  */}
          ...
          <div className="w-full text-center">
             <div className="btn m-2 p-2 bg-orange-400 hover:bg-orange-600 active:bg-orange-800 text-center px-5 text-white hover:text-white hover:text-lg" onClick={()=>handleSetData(data)}>Confirm Order</div>
          </div>
          </div>
        </DialogContent>
       </Dialog>
