...
const Invoice = ({price}) => {
  ...
  return (
                ...
                data.map((data, key)=>(
                    <>
                     <tr key={key}>
                       ...
                       <td className='text-center cursor-pointer '><span className='hover:text-red-600 hover:bg-black active:bg-green-400 cursor-pointer px-2 py-1 rounded-md text-center bg-red-400 font-bold' onClick={()=>teraLalGhagra(data.id)} >X</span></td>
                    </tr>
                    </>
                )
              )}
    ...
    <div className=" w-full flex flex-row justify-between">
             <span className="bg-yellow-500 border rounded-bl-lg rounded-tr-lg px-3 py-2 font-semibold cursor-pointer hover:bg-yellow-600 text-lg text-white" >Total amount : Rs. </span>
